...
async function buildMarketFallback() {
  ...
  const signals = (ranking.rankedMarkets || []).slice(0, MARKET_FALLBACK_LIMIT).map(m => ({ symbol: m.symbol, price: Number(m.lastPrice), status: "WAIT", valid: false, direction: null, entry: null, stop: null, targets: [], riskReward: null, quality: { score: Number(m.scores?.quality || 0), grade: "WATCH" }, stage: "market-ranking", reason: "Market ranked by Bybit while the trading scanner completes its next cycle.", lifecycle: null, generatedAt: new Date().toISOString(), source: "bybit", marketRank: m.rank, turnover24h: m.turnover24h, openInterest: m.openInterest, change24hPercent: Number(m.price24hPcnt || 0) * 100 }));
...
async function readScannerSnapshot() {
  ...
  if (Array.isArray(memory) && memory.length) return { signals: memory, scanResults: memory, scanner: { status: "LIVE", source: "scanner-memory", scannedSymbols: memory.length, publishedSignals: memory.filter(s => s.status === "READY" && s.valid === true).length, readySignals: memory.filter(s => s.status === "READY" && s.valid === true).length, waitSignals: memory.filter(s => s.status === "WAIT").length, errorSignals: memory.filter(s => s.status === "ERROR").length, updatedAt: new Date().toISOString() }, updatedAt: new Date().toISOString() };
  try {
    ...
    const signals = published.docs.map(doc => ({ ...doc.data(), signalId: doc.data().signalId || doc.id })).filter(s => LIVE_STATUSES.has(s.lifecycle?.status || s.signalState || s.status));
    const now = new Date().toISOString();
    return { signals, scanResults: signals, scanner: { status: signals.length ? "READY" : "WAITING", publishedSignals: signals.length, readySignals: signals.length, waitSignals: 0, errorSignals: 0, updatedAt: now }, updatedAt: now, recovered: true };
  } catch (error) {
    throw error;
  }
}
...
async function ensureScannerData(data) {
  ...
  try {
    ...
    const now = new Date().toISOString();
    return { ...data, signals: fallback, scanResults: fallback, scanner: { ...(data.scanner || {}), status: "WAITING", source: "bybit-market-fallback", scannedSymbols: 0, publishedSignals: 0, readySignals: 0, waitSignals: fallback.length, errorSignals: 0, updatedAt: now }, updatedAt: now };
  } catch (error) {
    return data;
  }
}
...
async function signalsRoutes(req, res) {
  ...
  return requireAuth(req, res, async () => {
    ...
    try {
      ...
      const responseSignals = await refreshLivePrices(source);
      ...
      let fallback = [];
      try { fallback = await buildMarketFallback(); } catch (fallbackError) {}
      return json(res, 200, { ok: true, data: { signals: fallback, scanResults: fallback, access, subscribeRequired: !access.hasAccess, scanner: { status: "DEGRADED", source: fallback.length ? "bybit-market-fallback" : "unavailable", scannedSymbols: 0, publishedSignals: 0, readySignals: 0, waitSignals: fallback.length, errorSignals: 0, updatedAt: new Date().toISOString() }, degraded: true } });
    }
  });
}
