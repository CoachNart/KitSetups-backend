...
// Compatibility helper used by multiple handlers.
async function readJsonBody(req) {
  // Reuse the existing readBody which already parses JSON and enforces size limits.
...
function readSubscriptions() {
  try {
    ...
    // Migration: older versions stored subscriptions directly under users[userId] as an array.
    // If we detect that shape, move it to parsed.subscriptions[userId] and create a minimal user object.
    const now = new Date().toISOString();
    for (const [key, val] of Object.entries(parsed.users)) {
      if (Array.isArray(val)) {
        parsed.subscriptions[key] = val;
        parsed.users[key] = {
          id: key,
          plan: parsed.users[key]?.plan || "free",
          createdAt: parsed.users[key]?.createdAt || now,
          trialStartedAt: parsed.users[key]?.trialStartedAt || now,
        };
      }
    }
...
async function handleRequest(req, res) {
  ...
  if (
    ...
    try {
      ...
      if (!Array.isArray(store.subscriptions[userId])) {
        store.subscriptions[userId] = [];
      }

      const exists = store.subscriptions[userId].some(
        (item) => item.endpoint === subscription.endpoint
      );

      if (!exists) {
        store.subscriptions[userId].push(subscription);
        writeSubscriptions(store);
      }
